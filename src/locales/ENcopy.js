...
export default {
  ...
  confirmTrMsg:
    'you are about to send <b>{amount}</b>DBC to address: <br> <b>{address}</b>',
  ...
  msg: {
    ...
    dlg_5:
      '1.确认支付后，DBC将打入深脑链的智能合约中，1小时以内退订机器只收取实际产生费用的20%，超过1小时主动退订机器，将收取实际费用的2倍，最多收取合约中所有DBC（此费用是深脑链智能合约收取，非dbchain平台收取）\n' +
      '<br>' +
  ...
  home: {
    intr_1:
      'DBC  是区块链技术驱动的开源人工智能计算平台，分布在全球的GPU通过DBC网络连接在一起。\n' +
      '\n' +
    ...
    intr_3:
      '在这个DBC网络购买机器需要使用DBC Token进行支付。支付通过智能合约完成，没有任何第三方参与。',
    intr_4:
      'DBC是区块链技术驱动的开源人工智能计算平台，分布在全球的GPU通过DBC网络连接在一起。',
    intr_5:
      '在DBC平台你可以找到最便宜的GPU。当你进行AI训练的时候，所有的数据都是加密保存和加密传输的',
    intr_6:
      'gpu租用按照分钟计费，GPU使用过程中如果出现中断，数据和模型将会自动迁移到同等配置的机器继续训练。不影响训练体验',
    intr_7:
      '深脑链总共有40亿的代币用于挖矿，前3年每年产出5亿，3年后，每年产出2.5亿，并且以后每5年产出减半。',
  ...
  miner: {
    ...
    rent: '出租机器',
    rentMachine: '打开或者创建钱包租用机器',
    addMc_msg_1: ['安装DBC挖矿软件', '请根据操作手册进行安装.'],
    addMc_msg_2: '安装完成后，会生成一个挖矿私钥，输入后才能在平台上绑定机器。',
    addMc_msg_3: '请输入挖矿私钥',
    addMc_msg_4: ['每个GPU定价', '存储每个GB定价'],
    addMc_msg_5:
      '收入将会扣除8%的手续费（其中2%用于DBC共识节点奖励，6%会被燃烧掉）',
    addMc_msg_6: [
      '如果机器使用过程中出现中断，则将会把合约中质押的7天机器使用量的DBC扣除。 出租时间不超过7天，则',
      '按照实际使用时间全部扣除！'
    ],
    mc_msg_1:
      '如果机器使用过程中出现中断，则将会把合约中质押的7天机器使用量的DBC扣除。<br>出租时间不超过7天，则全部扣除'
  },
  mcList: {},
  ...enLocale
};
