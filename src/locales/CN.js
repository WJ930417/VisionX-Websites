...
export default {
  ...
  msg: {
    ...
    wallet_4: "私钥不正确！",
    wallet_5:
      "一定要记住密码！不能丢失，" + "你需要这个密码和你的私钥才能解锁钱包 "
  },
  home: {
    ...
    intr_1:
      "提供GPU出租服务，加入我们，可以帮助您的机器非常方便的接入到深脑链网络中，获取收益。\n" +
      "\n" +
      "       只要你的GPU有空闲，就可以加入我们赚取DBC Token，同时可以享受Token升值带来的更多收益。\n" +
      "\n" +
      "      只要GPU闲置1小时以上就可以加入我们获取收益",
    intr_2:
      "这里的GPU来自于全球各地，通过DBC分布式网络连接在一起，这里面没有中心化服务器，你的数据不会被窃取。",
    intr_3:
      "在这个DBC网络购买机器需要使用DBC Token进行支付。GPU是按照美元定价，DBC价格越高需要的数量越少。",

    intr_4:
      "致力于为用户提供高性价比的人工智能算力服务，采用DBC TOKEN进行结算。深脑链是一个分布在全球的去中心化人工智能算力网络，通过我们您可以方便地使用网络中的机器，我们提供丰富的服务满足您的需求",
    intr_5:
      "在我们平台中你可以找到便宜且类型多样的GPU。当你使用的时候，所有的数据都是加密传输的",
    intr_6:
      "GPU租用按照分钟计费，GPU使用过程中如果出现中断，将会最多补偿144小时的费用",
    intr_7:
      "深脑链从2020年开始每年挖矿产出5亿，3年后，每年产出2.5亿，并且以后每5年产出减半，总计有40亿",
  ...
  miner: {
    ...
    addMc_msg_2:
      "安装完成后，需要填入当前的钱包地址，然后才能在平台上绑定机器。",
    ...
    mc_msg_1:
      "如果机器使用过程中出现中断，则将会把质押的144小时机器使用量的DBC扣除。<br>出租时间不超过144小时，则全部扣除"
  },
  mcList: {},
  ...
  wallet_box_adddress:
    "钱包地址:您的地址也可以称为您的帐户.当你把这个地址发给对方时,对方可以给您的钱包地址打DBC代币.",
  wallet_box_keystore:
    "密钥存储文件:密钥存储文件使用密码以加密形式存储您的私钥.建议始终使用密钥存储文件来解锁您的钱包.",
  wallet_box_key:
    "您的私钥这是您的私钥的未加密文本版本,意味着不需要密码.如果有人找到你的未加密的私钥,他们可以在没有密码的情况下访问你的钱包.因此,通常建议使用加密版本.",
  ...
  my_machine_miner_tips:
    "如果机器使用过程中出现中断，则将会把合约中质押的7天机器使用量的DBC扣除。出租时间不超过7天，则全部扣除",
