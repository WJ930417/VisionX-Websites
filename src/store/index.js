...
export default new Vuex.Store({
  ...
  actions: {
    getExchangeRate({ commit, state }) {
      axios
        .get("https://api.coinmarketcap.com/v1/ticker/deepbrain-chain/")
        .then(res => {
    ...
    getRateForUSDToCNY({ commit, state }) {
      axios
        .get(
          "http://apilayer.net/api/live?access_key=e0117300e4cb280db0e4e2fc86edddb0&currencies=CNY"
        )
        .then(res => {
          console.log(res.data[0]);
          commit("setUSDToCNY", res.data.quotes.USDCNY);
        })
        .catch(err => {});
    },
    ...
    createAccountFromEncryptedKey(
      ...
      return initAccountFromEncryptedKey(encryptedKey, password).then(
        account => {
          saveCookie(account);
          commit("setData", {
            privateKey: account.privateKey,
