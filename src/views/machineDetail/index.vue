    ...
    <div class="border-box">
      <div class="title">
        ...
        <div v-if="mcData" class="flex right vCenter">
          ...
          <el-rate
            :value="mcData.evaluation_score_average / 2"
            disabled
          ></el-rate>
      ...
      <table class="tb-theme hAuto">
        ...
        <tbody>
          <tr v-for="item in content">
            ...
            <td>{{ item.order_isnormal_over ? 'N' : 'Y' }}</td>
            <td>{{ item.gpu_count }}</td>
            <!--          <td>24d 34h</td>-->
            <td>{{ $secToDate(item.rent_time_length * 60, 'DH') }}</td>
            <td>
              <div class="flex right vCenter">
                <el-rate :value="item.evaluation_score / 2" disabled></el-rate>
                <span>{{ item.evaluation_score }}</span>
              </div>
              <div class="rate-wrap">
